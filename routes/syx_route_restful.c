...
zend_class_entry *syx_route_restful_ce;

/** {{{ ARG_INFO
 */
ZEND_BEGIN_ARG_INFO_EX(syx_route_restful_construct_arginfo, 0, 0, 2)
...
syx_route_t * syx_route_restful_instance(syx_route_t *this_ptr, zval *match, zval *route, zval *verify) /* {{{ */ {
	if (Z_ISUNDEF_P(this_ptr)) {
		object_init_ex(this_ptr, syx_route_restful_ce);
	}

	zend_update_property(syx_route_restful_ce, this_ptr, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_MATCH), match);
	zend_update_property(syx_route_restful_ce, this_ptr, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_ROUTE), route);

	if (!verify) {
		zend_update_property_null(syx_route_restful_ce, this_ptr, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_VERIFY));
	} else {
		zend_update_property(syx_route_restful_ce, this_ptr, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_VERIFY), verify);
	}
...
/** {{{ static int syx_route_restful_match(syx_route_t *router, zend_string *uri, zval *ret)
 */
static int syx_route_restful_match(syx_route_t *router, zend_string *uri, zval *ret) {
	...
	match  = zend_read_property(syx_route_restful_ce, router, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_MATCH), 0, NULL);
...
/** {{{ int syx_route_restful_route(syx_route_t *router, syx_request_t *request)
 */
int syx_route_restful_route(syx_route_t *router, syx_request_t *request) {
	zend_string *request_uri;
	zval args, *base_uri, *zuri, *request_method, *route_method, *routes;

	routes = zend_read_property(syx_route_restful_ce, router, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_ROUTE), 1, NULL);
	request_method = zend_read_property(syx_request_ce, request, ZEND_STRL(SYX_REQUEST_PROPERTY_NAME_METHOD), 1, NULL);
	if ((route_method = zend_hash_str_find(Z_ARRVAL_P(routes), ZEND_STRL("method"))) != NULL && IS_STRING == Z_TYPE_P(route_method) && strncasecmp(Z_STRVAL_P(route_method), Z_STRVAL_P(request_method), Z_STRLEN_P(request_method))) {
        return 0;
	}
	...
	if (!syx_route_restful_match(router, request_uri, &args)) {
...
/** {{{ proto public Syx_route_restful::route(Syx_Request_Abstract $request)
 */
PHP_METHOD(syx_route_restful, route) {
	syx_route_t 	*route;
	...
	RETURN_BOOL(syx_route_restful_route(route, request));
}
/** }}} */

/** {{{ zend_string * syx_route_restful_assemble(syx_route_t *this_ptr, zval *info, zval *query)
 */
zend_string * syx_route_restful_assemble(syx_route_t *this_ptr, zval *info, zval *query) {
	...
	match  = zend_read_property(syx_route_restful_ce, this_ptr, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_MATCH), 0, NULL);
...
/** {{{ proto public Syx_route_restful::match(string $uri)
 */
PHP_METHOD(syx_route_restful, match) {
	zend_string *uri;
	zval matches;
	...
	if (syx_route_restful_match(getThis(), uri, &matches)) {
...
/** {{{ proto public Syx_route_restful::__construct(string $match, array $route, array $verify, string $reverse = NULL)
 */
PHP_METHOD(syx_route_restful, __construct) {
	...
	(void)syx_route_restful_instance(self, match, route, verify);
...
/** {{{ proto public Syx_route_restful::assemble(array $info[, array $query = NULL])
*/
PHP_METHOD(syx_route_restful, assemble) {
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "a|a", &info, &query) == FAILURE) {
		...
		zend_string *str;
	    if ((str = syx_route_restful_assemble(getThis(), info, query)) != NULL) {
			RETURN_STR(str);
		}
		RETURN_NULL();
	}
...
/** {{{ syx_route_restful_methods
 */
zend_function_entry syx_route_restful_methods[] = {
	PHP_ME(syx_route_restful, __construct, syx_route_restful_construct_arginfo, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
	PHP_ME(syx_route_restful, route, syx_route_route_arginfo, ZEND_ACC_PUBLIC)
	PHP_ME(syx_route_restful, assemble, syx_route_assemble_arginfo, ZEND_ACC_PUBLIC)
    {NULL, NULL, NULL}
};
/* }}} */
 ...
 */
SYX_STARTUP_FUNCTION(route_restful) {
	zend_class_entry ce;
	SYX_INIT_CLASS_ENTRY(ce, "Syx\\Route\\Restful", syx_route_restful_methods);
	syx_route_restful_ce = zend_register_internal_class_ex(&ce, NULL);
	zend_class_implements(syx_route_restful_ce, 1, syx_route_ce);
	syx_route_restful_ce->ce_flags |= ZEND_ACC_FINAL;

	zend_declare_property_null(syx_route_restful_ce, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_MATCH),  ZEND_ACC_PROTECTED);
	zend_declare_property_null(syx_route_restful_ce, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_ROUTE),  ZEND_ACC_PROTECTED);
	zend_declare_property_null(syx_route_restful_ce, ZEND_STRL(SYX_ROUTE_PROPETY_NAME_VERIFY), ZEND_ACC_PROTECTED);
